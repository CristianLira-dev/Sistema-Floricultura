...
<html lang="pt-br">
  <head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Floricultura Jardim  | Consulta Fornecedores</title>
...
<div class="modal fade" id="erro" aria-labelledby="errorModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    ...
    <header>
      ...
      <div>
        <h5 class="text-light">Consulta Fornecedores</h5>
      </div>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $buscar == "") {
 ...
 if (!empty($buscar)) {
    $sql = "SELECT * FROM fornecedores WHERE 
      ...
      razao_social LIKE :busca OR 
      nome_fantasia LIKE :busca OR 
      cnpj LIKE :busca OR 
      cep LIKE :busca OR 
    ...
    $select = $conn->prepare("SELECT * FROM fornecedores");
    $select->execute();
  }
  ...
  if ($resultados > 0) {
    while($row = $select->fetch()) {
      ...
      echo "<br><b>Razão social: </b>" . destacar($row['razao_social'], $buscar);
      echo "<br><b>Nome Fantasia: </b>" . destacar($row['nome_fantasia'], $buscar);
      echo "<br><b>CNPJ: </b>" . destacar($row['cnpj'], $buscar);
      ...
      echo "<br><b>Email: </b>" . destacar($row['email'], $buscar);
         echo "<div style='display:flex; margin-left: 10px; gap: 5px;'>";
      echo "<br><br><a class='button' href='config/alterar_fornecedor.php?codigo=" . urlencode($row['codigo']) . "'>Alterar</a>";
      echo "<br><br><a class='button' href='config/delete_fornecedores.php?codigo=" . urlencode($row['codigo']) . "'>Excluir</a>";
         echo "</div>";
      echo "<hr>";
    }
  }
