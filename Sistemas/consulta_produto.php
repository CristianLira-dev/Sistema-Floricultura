...
<html lang="pt-br">
  <head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Floricultura Jardim  | Consulta Produtos</title>
...
<div class="modal fade" id="erro" aria-labelledby="errorModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    ...
    <header>
      ...
      <div>
        <h5 class="text-light">Consulta Produtos</h5>
      </div>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $buscar == "") {
 ...
 if (!empty($buscar)) {
    $sql = "SELECT * FROM produtos WHERE 
      ...
      peso LIKE :busca OR 
      codigo_barras LIKE :busca OR 
      preco_custo LIKE :busca OR 
      preco_venda LIKE :busca OR 
      estoque LIKE :busca OR 
      estoqueMax LIKE :busca OR 
      marca LIKE :busca OR 
  ...
  if ($resultados > 0) {
    while($row = $select->fetch()) {
      ...
      echo "<br><b>Descrição do Produto: </b>" . destacar($row['descricao'], $buscar);
      echo "<br><b>Peso: </b>" . destacar($row['peso'], $buscar);
      echo "<br><b>Código de Barras: </b>" . destacar($row['codigo_barras'], $buscar);
      echo "<br><b>Preço do Custo: </b>" . destacar($row['preco_custo'], $buscar);
      echo "<br><b>Preço da Venda: </b>" . destacar($row['preco_venda'], $buscar);
      echo "<br><b>Estoque Atual: </b>" . destacar($row['estoque'], $buscar);
      echo "<br><b>Estoque Máximo: </b>" . destacar($row['estoqueMax'], $buscar);
      echo "<br><b>Marca: </b>" . destacar($row['marca'], $buscar);
      echo "<br><b>Fornecedor: </b>" . destacar($row['fornecedor'], $buscar);
      echo "<br><b>Setor: </b>" . destacar($row['setor'], $buscar);
      echo "<div style='display:flex; margin-left: 10px; gap: 5px;'>";
      echo "<br><br><a class='button' href='config/alterar_produto.php?codigo=" . urlencode($row['codigo']) . "'>Alterar</a>";
      echo "<br><br><a class='button' href='config/delete_produto.php?codigo=" . urlencode($row['codigo']) . "'>Excluir</a>";
      echo "</div>";
      echo "<hr>";
    }
  }
