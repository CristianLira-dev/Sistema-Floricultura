...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Floricultura Jardim | Consulta Clientes</title>
...
<header>
  ...
  <form class="d-flex" method="POST" action="#" role="search">
    <input class="form-control me-2" type="search" name="buscar" placeholder="digite..." aria-label="Search" style="outline: none; box-shadow: none;" value="<?php echo isset($_POST['buscar']) ? htmlspecialchars($_POST['buscar']) : ''; ?>">
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $buscar == "") {
 ...
 if (!empty($buscar)) {
    ...
    $select = $conn->prepare("SELECT * FROM clientes");
    $select->execute();
  }
  ...
  if ($resultados > 0) {
    while($row = $select->fetch()) {
      ...
      echo "<br><b>RG: </b>" . destacar($row['rg'], $buscar);
      ...
      echo "<br><br><a class='button p-2' href='config/alterar_clientes.php?codigo=" . urlencode($row['codigo']) . "'>Alterar</a>";
      echo "<br><br><a class='button p-2' href='config/delete_cliente.php?codigo=" . urlencode($row['codigo']) . "'>Excluir</a>";
      echo "</div>";
      echo "<hr>";
    }
  }
