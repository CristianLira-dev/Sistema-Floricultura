...
<html lang="pt-br">
  <head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Floricultura Jardim  | Consulta Usuario</title>
...
<div class="modal fade" id="erro" aria-labelledby="errorModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    ...
    <header>
      ...
      <div>
        <h5 class="text-light">Consulta Usuario</h5>
      </div>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $buscar == "") {
 ...
 if (!empty($buscar)) {
    ...
    $select = $conn->prepare("SELECT * FROM usuarios");
    $select->execute();
  }
  ...
  if ($resultados > 0) {
    while($row = $select->fetch()) {
      ...
      echo "<br><b>Função: </b>" . destacar($row['funcao'], $buscar);
      echo "<br><b>Senha: </b>" . destacar($row['senha'], $buscar);
      echo "<br><b>Email: </b>" . destacar($row['email'], $buscar);
      echo "<div style='display:flex; margin-left: 10px; gap: 5px;'>";
      echo "<br><br><a class='button' href='config/alterar_usuario.php?codigo=" . urlencode($row['codigo']) . "'>Alterar</a>";
      echo "<br><br><a class='button' href='config/delete_usuario.php?codigo=" . urlencode($row['codigo']) . "'>Excluir</a>";
      echo "</div>";
      echo "<hr>";
    }
  }
